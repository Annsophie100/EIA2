...
namespace a07_eisdealer {
    ...
    function checkWarenkorb(_event: Event): void {
        ...
        if (checkAdresse == true && checkArtikel == true) {
            document.getElementById("confirm").addEventListener("click", bestellen);
        }
    ...
    // new shit mit url__________________________________________________________
    
    function bestellen(): void {
        let urlSchreiben: string = "https://eiaannsophie.herokuapp.com/?";
        console.log("sortenListe");

        let inputsAlle: HTMLCollectionOf<HTMLInputElement> = document.getElementsByTagName("input");
        for (let input of inputsAlle) {
            if (input.type == "number" && parseFloat(input.value) != 0) {
                urlSchreiben += `${input.getAttribute("produkt") + input.name}=${input.value}&`;
            }
            if (input.checked == true)
                urlSchreiben += `${input.getAttribute("produkt")}=${input.value}&`;
            if (input.type == "text")
                urlSchreiben += `${input.name}=${input.value}&`;
        }
