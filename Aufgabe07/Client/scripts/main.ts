...
namespace a07_eisdealer {
    ...
    function warenkorbSchreiben(): void {
        ...
        if (writeWarenkorb == 0) {
            ...
            let checkButton: HTMLButtonElement = document.createElement("button");
    ...
    function urlSchreiben(): void {
        let url: string = "https://eiaannsophie.herokuapp.com/?";

        let inputsAlle: HTMLCollectionOf<HTMLInputElement> = document.getElementsByTagName("input");
        for (let input of inputsAlle) {

            let preisEinfach: number = parseFloat(input.getAttribute("preis"));

            //für stepper
            if (input.type == "number" && parseFloat(input.value) > 0) {
                let preisMehrfach: number = preisEinfach * parseFloat(input.value);
        ...
        let endpreis: string = document.getElementById("gesamtpreis").getAttribute("preis");
        url += `Endpreis:`;
        url += endpreis;
        url += `Euro`;
